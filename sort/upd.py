...
def json_data(file_path):
    with open(file_path, 'r') as file:
        data = json.load(file)
        return {key: data[key] for key in ('screen_name', 'name', 'description', 'followed_by', 'following')}
...
for filename in os.listdir(source_dir):
    if filename.endswith('.json'):
        ...
        if os.path.isfile(target_file_path):
            ...
            target_data = {key: target_data[key] for key in ('screen_name', 'name', 'description', 'followed_by', 'following')}
